 ...
 */
public class FilePickerPopup extends PopupScreen {
	String _currentPath;
	String[] _extensions;
	 ...
	 * 
	 * @param startPath
	 *            The initial path to open. Use null to start at the root file systems.
	 * @param extensions
	 *            Allowable file extensions to display. Use null to display all file types.
	 */
	public FilePickerPopup(String startPath, String[] extensions) {
				...
				while (fileEnum.hasMoreElements()) {
					...
					if (_extensions == null) {
						...
						if (currentFile.lastIndexOf('/') == (currentFile.length() - 1)) {
							...
							// This is a file; check if its extension matches
							// the filter
	...
	private void doSelection() {
		...
		if (thePath == null) {
			...
			// The user has selected a file; close the screen
			this.close();
		}
	}
	...
	// If a file has been selected, close this screen. Returns *?* if the user
	// has selected a file.
	private String buildPath() {
		String newPath = (String) _olf.get(_olf, _olf.getSelectedIndex());

		if (newPath.equals("..")) {
			// Go up a directory; remove the trailing '/'
			newPath = _currentPath.substring(0, _currentPath.length() - 2);

			// Remove everything after the last '/' (the current directory)
			// If a '/' is not found, the user is opening the file system root
			// return null to cause the screen to display the file system root
			...
			// prefix the _currentPath if it is not null (not in the root
			// directory)
	...
	// Saves the files and directories listed in vector format into an object
	// array
